...
class Model(object):
	...
	def __init__(self, labId, model_type = 'lstm', train_data_dir = '', val_size = 0.1):
		...
		self.trainX, self.valX, self.trainY, self.valY, self.tokenizer, self.embedding_matrix = get_train_data(train_data_dir, val_size)
		# create model
		self.model = get_model(tokenizer=self.tokenizer, embedding_matrix = self.embedding_matrix, rnn_cell= model_type)
